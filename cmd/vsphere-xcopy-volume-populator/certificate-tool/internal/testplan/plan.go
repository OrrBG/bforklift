...
func (tp *TestPlan) Start(ctx context.Context, clientset *kubernetes.Clientset, namespace, namespaceId, podImage, storageClassName, pvcYamlPath string) error {
	for i := range tp.TestCases {
		tc := &tp.TestCases[i]
		start := time.Now()
		if err := tc.Run(ctx, clientset, namespace, namespaceId, podImage, tp.Image, storageClassName, pvcYamlPath, tp.StorageVendorProduct); err != nil {
