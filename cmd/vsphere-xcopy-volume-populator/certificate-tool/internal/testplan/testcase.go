...
func (tc *TestCase) Run(ctx context.Context, clientset *kubernetes.Clientset, namespace, namespaceId, podImage, vmImage, storageClassName, pvcYamlPath, storageVendorProduct, secretName string) error {
	if err := ensureVMs(tc.Name, vmImage, tc.VMs); err != nil {
		return fmt.Errorf("VM setup failed: %w", err)
	}

	namespace = namespace + "-" + namespaceId
	tc.Name = tc.Name + "-" + namespaceId
	secretName = secretName + "-" + namespaceId
	for _, vm := range tc.VMs {
		pvcName := fmt.Sprintf("pvc-%s", tc.Name)
		...
		podName := fmt.Sprintf("populator-%s", tc.Name)
		if err := k8s.EnsurePopulatorPod(ctx, clientset, namespace, podName, podImage, tc.Name, *vm, storageVendorProduct, pvcName, secretName); err != nil {
