...
func (c *NetappClonner) Map(initatorGroup string, targetLUN populator.LUN) error {
	lunPath := fmt.Sprintf("/vol/%s/lun0", targetLUN.Name)
	_, err := c.api.EnsureLunMapped(context.TODO(), initatorGroup, lunPath)
	if err != nil {
		return fmt.Errorf("Failed to map lun path %s to group %s: %w ", lunPath, initatorGroup, err)
	}
	return err
}

func (c *NetappClonner) UnMap(initatorGroup string, targetLUN populator.LUN) error {
	return c.api.LunUnmap(context.TODO(), initatorGroup, fmt.Sprintf("/vol/%s/lun0", targetLUN.Name))
}
...
func (c *NetappClonner) ResolveVolumeHandleToLUN(volumeHandle string) (populator.LUN, error) {
	...
	lun := populator.LUN{Name: volumeHandle, SerialNumber: serialNumber, ProviderID: OntapProviderID}
	return lun, nil
}
...
func (c *NetappClonner) CurrentMappedGroups(targetLUN populator.LUN) ([]string, error) {
	lunPath := fmt.Sprintf("/vol/%s/lun0", targetLUN.Name)
	lunMappedIgroups, err := c.api.LunListIgroupsMapped(context.Background(), lunPath)
	if err != nil {
		return nil, fmt.Errorf("Failed to get mapped luns by path %s: %w ", lunPath, err)
	}
